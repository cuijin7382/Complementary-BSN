...
from ..util.util2 import human_format, np2tensor, rot_hflip_img, psnr, ssim, tensor2np, imread_tensor, lpips2, dists2
...
class BaseTrainer():
    ...
    def __init__(self, cfg):
        self.session_name = cfg['session_name']  # session name和denoiser的关系是什么
    ...
    def _warmup(self):
        ...
        for key in self.train_dataloader:
            self.train_dataloader_iter[key] = iter(self.train_dataloader[key])  # 具体的加载在哪

        warmup_iter = self.train_cfg['warmup_iter']
        if warmup_iter > self.max_iter:
            self.logger.info(
                'currently warmup support 1 epoch as maximum. warmup iter is replaced to 1 epoch iteration. %d -> %d' \
    ...
    def _before_test(self, dataset_load):
        ...
        # test dataset loader
        if dataset_load:
            self.test_dataloader = self._set_dataloader(self.test_cfg, batch_size=1, shuffle=False,
                                                        num_workers=self.cfg['thread'])
    ...
    def _before_train(self):
        ...
        self.train_dataloader = self._set_dataloader(self.train_cfg, batch_size=self.train_cfg['batch_size'],
                                                     shuffle=True, num_workers=self.cfg['thread'])  # 加载数据

        # validation dataset loader
        if self.val_cfg['val']:
            self.val_dataloader = self._set_dataloader(self.val_cfg, batch_size=1, shuffle=False,
                                                       num_workers=self.cfg['thread'])

        # other configuration
        self.max_epoch = self.train_cfg['max_epoch']
        self.epoch = self.start_epoch = 1
        max_len = self.train_dataloader[
            'dataset'].dataset.__len__()  # base number of iteration works for dataset named 'dataset'
        ...
        if self.cfg["resume"]:
            ...
            self.logger = Logger((self.max_epoch, self.max_iter), log_dir=self.file_manager.get_dir(''),
                                 log_file_option='a')
        else:
            # logger initialization
            self.logger = Logger((self.max_epoch, self.max_iter), log_dir=self.file_manager.get_dir(''),
    ...
    def _run_step(self):
        ...
        # backward
        # loss1=sum(v for v in losses1.values())
        total_loss = sum(v for v in losses.values())
        total_loss.requires_grad_(True)  # 提示错误element 0 of tensors does not require grad and does not have a grad_fn
        # loss = weight* criterion1(output, y1) + (1-weight) * criterion2(output, y2)
        total_loss.backward()
    ...
    def test_dataloader_process(self, dataloader, add_con=0., floor=False, img_save=True, img_save_path=None,
                                info=True):
        '''
        ...
        for idx, data in enumerate(dataloader):
            ...
            if 'clean' in data:
                ...
                lpips_value = lpips2(denoised_image, data['clean'])
                dists_value = dists2(denoised_image, data['clean'])
                # psnrL.append(round(psnr_value, 2))
                # ssimL.append(round(ssim_value, 3))
                psnr_sum += psnr_value
                ssim_sum += ssim_value
                lpips_sum += lpips_value
                dists_sum += dists_value
                count += 1
            # print(psnrL)
            # print(ssimL)
            # image save
            if img_save:
                ...
                if 'real_noisy' in self.cfg['model_input']:
                    noisy_img = data['real_noisy']
                elif 'syn_noisy' in self.cfg['model_input']:
                    noisy_img = data['syn_noisy']
                elif 'noisy' in self.cfg['model_input']:
                ...
                denoi_name = '%04d_DN_%.2f_%.3f_%.3f_%.3f' % (
                idx, psnr_value, ssim_value, lpips_value, dists_value) if 'clean' in data else '%04d_DN' % idx
            ...
            if info:
                if 'clean' in data:
                    self.logger.note('[%s] testing... %04d/%04d. PSNR : %.2f dB' % (
                    self.status, idx, dataloader.__len__(), psnr_value), end='\r')
                else:
                    self.logger.note('[%s] testing... %04d/%04d.' % (self.status, idx, dataloader.__len__()), end='\r')

        # final log msg
        if count > 0:
            self.logger.val('[%s] Done! PSNR : %.2f dB, SSIM : %.3f,lpips : %.3f,dists : %.3f' % (
            self.status, psnr_sum / count, ssim_sum / count, lpips_sum / count, dists_sum / count))
        else:
        ...
        if count == 0:
            ...
            return psnr_sum / count, ssim_sum / count, lpips_sum / count, dists_sum / count
    ...
    def _set_denoiser(self):
        if hasattr(self.model['denoiser'].module, 'denoise'):
            self.denoiser = self.model['denoiser'].module.denoise  # denoise
        else:
    ...
    def print_loss(self):
        ...
        self.status, self.iter, self.max_iter, "{:.1e}".format(self._get_current_lr()))
    ...
    def load_checkpoint(self, load_epoch=0, name=None):
        if name is None:
            ...
            file_name = os.path.join(self.file_manager.get_dir(self.checkpoint_folder),
    ...
    def _set_dataloader(self, dataset_cfg, batch_size, shuffle, num_workers):
        ...
        for key in dataset_dict:
            ...
            dataloader[key] = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=shuffle,
    ...
    def _set_one_optimizer(self, opt, parameters, lr):
        ...
        if opt['type'] == 'SGD':
            return optim.SGD(parameters, lr=lr, momentum=float(opt['SGD']['momentum']),
                             weight_decay=float(opt['SGD']['weight_decay']))
        elif opt['type'] == 'Adam':
            return optim.Adam(parameters, lr=lr, betas=opt['Adam']['betas'])
        elif opt['type'] == 'AdamW':
            return optim.Adam(parameters, lr=lr, betas=opt['AdamW']['betas'],
                              weight_decay=float(opt['AdamW']['weight_decay']))
        else:
